...
```
Data used here comes from the [International Union for Conservation of Nature (IUCN) Red List of Threatened Species (Version 2020-1)](https://www.iucnredlist.org/). <br>It was repared by [Florent Lavergne](https://www.behance.net/florentlavergne).
...
ggplot(data = summary_threats, 
 ...
 #                    y = reorder(threat_type, tot_threatened))) +
  ...
  labs(x = '# of threatened plant species worldwide', y = '', size = '') +
  guides(size = FALSE)
...
The visualization below includes only countries with more than 5 threatened species.
...
ggplot(data = summary_country_threat %>% filter(tot_threatened > 5), 
  ...
  labs(x = '# of threatened plant species', y = '', colour = 'continent') +
  geom_point(size = 3)
...
ggplot(data = summary_actions, 
  ...
  labs(x = '# of actions reports worldwide', y = '', size = '') +
  guides(size = FALSE)
...
The visualization below includes only countries which have taken more than 5 actions and does not include reports in which the action type was unknown.
...
ggplot(data = summary_country_action %>% filter(tot_action > 3),
       mapping = aes(x = tot_action,
                     ...
                     colour = factor(continent))) +
  labs(x = '# of actions taken', y = '', colour = 'continent') +
  geom_point(size = 3)
